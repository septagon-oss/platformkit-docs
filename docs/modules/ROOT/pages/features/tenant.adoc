...
| `tenant:manage` | the operator's tenant only | `list`, `create`, `read`, `add-host`, `invite`, `suspend`
|===
...
| `tenant.created` | `create` | `audit` (every event)
| `tenant.suspended` | `suspend` | `audit` (every event)
| `tenant.host_added` | `add-host` | `audit` (every event)
| `user.invited` | `invite` | `auth`, `audit` (every event)
|===
